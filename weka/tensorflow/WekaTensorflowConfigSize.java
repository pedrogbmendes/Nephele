...
public class WekaTensorflowConfigSize extends TensorflowConfigSize implements WekaConfiguration{
	...
	final static Integer[] nr_worker_cores = {8, 16, 32, 48, 64, 80};
	final static Integer[] ps			   = {1};
	final static Double [] S			   = {1.0/60.0, 6.0/60.0, 15.0/60.0, 3.0/6.0, 6.0/6.0}; 
	
	/* class constructors */
	public WekaTensorflowConfigSize(int nr_ps, int nr_instances, double lr, int bs, int synchronism, int flavor, double size, Instances d) {
		super(nr_ps, nr_instances, lr, bs, synchronism, flavor, size);
		dataset = d;
		if (this.dataset == null)
	    	throw new RuntimeException("[WekaTensorflowConfigSize] Cannot be null");
	}
	
	public WekaTensorflowConfigSize(Instance i){
		super((int) i.value(0), (int) i.value(1), (double) i.value(2), (int) i.value(3), (int)i.value(4), (int)i.value(5), (double)i.value(6));
		this.dataset = i.dataset();
	    if (this.dataset == null)
	    	throw new RuntimeException("[WekaTensorflowConfigSize] Cannot be null");
	}

	public WekaTensorflowConfigSize(TensorflowConfigSize wekaTensorflow, Instances dataset) {
		super(wekaTensorflow.getNr_ps(), wekaTensorflow.getNr_workers(), wekaTensorflow.getLearning_rate(), wekaTensorflow.getBatch_size(), wekaTensorflow.getSynchronism(), wekaTensorflow.getVm_flavor(), wekaTensorflow.getDataset_size());
...
//	   return rr;
        ...
        rr.setValue(6, (double) this.getDataset_size());
        return rr;
		
	}

	@Override
	public WekaTensorflowConfigSize clone() {
	   return  new WekaTensorflowConfigSize(this,this.dataset);   //NB: this is *not* copying the instances
	}
	...
	public ArrayList<WekaConfiguration> neighbourhood(){
		   ...
		   if (index >= 0 && index < ps.length-1){	// generate next
			   neighbour = new WekaTensorflowConfigSize(ps[index + 1], this.nr_workers, this.learning_rate, this.batch_size, this.synchronism, this.vm_flavor, this.s, this.dataset);
		   ...
		   if (index >= 0 && index < nr_worker_cores.length - 1){	// generate next
			   neighbour = new WekaTensorflowConfigSize(this.nr_ps, nr_worker_cores[index + 1]/nrCores(this.vm_flavor), this.learning_rate, this.batch_size, this.synchronism, this.vm_flavor, this.s, this.dataset);
			   neighbourhood.add(neighbour);
		   }
		   ...
		   if (index >= 0 && index < lr.length - 1){	// generate neighbour forward
			   neighbour = new WekaTensorflowConfigSize(this.nr_ps, this.nr_workers, lr[index + 1], this.batch_size, this.synchronism, this.vm_flavor, this.s, this.dataset);
			   neighbourhood.add(neighbour);
		   }
		   ...
		   if (index >= 0 && index < bs.length - 1){	// generate neighbour forward
			   neighbour = new WekaTensorflowConfigSize(this.nr_ps, this.nr_workers, this.learning_rate, bs[index + 1], this.synchronism, this.vm_flavor, this.s, this.dataset);
			   neighbourhood.add(neighbour);
		   } 
		   ...
		   if (index >= 0 && index < sync.length - 1){	// generate neighbour forward
			   neighbour = new WekaTensorflowConfigSize(this.nr_ps, this.nr_workers, this.learning_rate, this.batch_size, sync[index + 1], this.vm_flavor, this.s, this.dataset);
			   neighbourhood.add(neighbour);
		   } 
		   ...
		   if (index >= 0 && index < flavor.length - 1){	// generate neighbour forward
			   neighbour = new WekaTensorflowConfigSize(this.nr_ps, (this.nr_workers*nrCores(this.vm_flavor))/nrCores(flavor[index + 1]), this.learning_rate, this.batch_size, this.synchronism, flavor[index + 1], this.s, this.dataset);
			   neighbourhood.add(neighbour);
		   } 
		   ...
		   // dataset size
		   index = Arrays.asList(S).indexOf(this.s);
		   if (index >= 0 && index < S.length - 1){	// generate neighbour forward
			   neighbour = new WekaTensorflowConfigSize(this.nr_ps, (this.nr_workers*nrCores(this.vm_flavor))/nrCores(flavor[index + 1]), this.learning_rate, this.batch_size, this.synchronism, this.vm_flavor, S[index + 1], this.dataset);
			   neighbourhood.add(neighbour);
		   } 
		   if (index <= flavor.length - 1 && index > 0){	// generate backward
			   neighbour = new WekaTensorflowConfigSize(this.nr_ps, (this.nr_workers*nrCores(this.vm_flavor))/nrCores(flavor[index - 1]), this.learning_rate, this.batch_size, this.synchronism, this.vm_flavor, S[index - 1], this.dataset);
