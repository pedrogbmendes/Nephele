...
import static lynceus.scout.ExtendedScoutVMConfig.parseName;
 ...
 * @since 29.03.18
 */
public class ExtendedScoutVMCostGenerator implements CostGenerator<ExtendedScoutVMConfig> {
   ...
   private Map<ExtendedScoutVMConfig, Double> costMap = null;
   private Map<ExtendedScoutVMConfig, Double> timeMap = null;
   ...
   /* class constructor */
   public ExtendedScoutVMCostGenerator() {
      /* to write a file with the dataset info */
		   if (write_file) {
...
//      updateCostOnWorkload(targetWkld);
   ...
   @Override
   public double setupCost(State state, ExtendedScoutVMConfig config) {
      return 0.0D;
   }

   @Override
   public double deploymentCost(State state, ExtendedScoutVMConfig config) {
   ...
   @Override
   public double costPerConfigPerMinute(ExtendedScoutVMConfig config) {
      return AWSDirectory.costPerConfigPerMinute(config.getType(), config.getSize(), config.getNum_instances());

   }

   @Override
   public double getAccForSpecificTime(double time, ExtendedScoutVMConfig config) {
      // TODO Auto-generated method stub
      return 0;
   }

   @Override
   public double[] getIntermediateValues(ExtendedScoutVMConfig config, int amount, timeout timeoutType) {
      // TODO Auto-generated method stub
      return null;
   }
   
	@Override
	public double getAccForConfig(ExtendedScoutVMConfig config) {
		// TODO Auto-generated method stub
		return -1;
	}
   ...
   public static void setTargetWkld(int targetWkld) {
      ...
      try {
         ...
         ExtendedScoutVMCostGenerator.targetWkld = files.get(targetWkld - 1).getName();
   ...
   private void updateCostOnWorkload(String currWorkload) {
      ...
      try {
         ...
         for (File f : listOfFiles) {
            ...
            ExtendedScoutVMConfig config = parseName(f_name);
   ...
   public double getTimeForSpecificConstraint(double perc) {
      ...
      int i = 0;
      for (Map.Entry<ExtendedScoutVMConfig, Double> config : timeMap.entrySet()) {
