...
public class ReducedScoutLHS <C extends Configuration> extends LHS{

	/* class attributes */
	private ArrayList<double[]> dims;
	
	/* class constructor */
	public ReducedScoutLHS(int samples) {
		super(samples);
	}
	...
	protected Configuration newSample(int index) {
		...
		ReducedScoutVMConfig newConfig = new ReducedScoutVMConfig((int) num_instances, type, size, vcpus);
		//System.out.println("newConfig = " + newConfig);
		
		return new ReducedWekaScoutVMConfig(newConfig, super.dataset);
	}
	...
	protected ArrayList<Double> dimensions(Configuration config) {
		...
		int num_instances = (int) config.at(0);
		AWSDirectory.AWSInstanceType type = (AWSInstanceType) config.at(1);
