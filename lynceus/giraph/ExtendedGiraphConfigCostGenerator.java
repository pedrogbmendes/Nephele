...
public class ExtendedGiraphConfigCostGenerator implements CostGenerator<ExtendedGiraphConfig> {
	
	/* class attributes */
	private double avgCost;
	private double maxTime;
	private double minTime;
	private Map<ExtendedGiraphConfig, Double> costMap = null;
	private Map<ExtendedGiraphConfig, Double> timeMap = null;
	private String dataset;
	
	/* class constructor */
	public ExtendedGiraphConfigCostGenerator(String dataset) throws IOException{
	...
	@Override
	public double setupCost(State state, ExtendedGiraphConfig config) {
		// TODO Auto-generated method stub
		return 0;
	}

	@Override
	public double deploymentCost(State state, ExtendedGiraphConfig config) {
		 final Double cost = costMap.get(config);

	      if (cost == null)
	         throw new RuntimeException("[ExtendedGiraphConfigCostGenerator] " + config + " with hashcode " + config.hashCode() + " not in cost map for wkld " + this.dataset + " map size " + costMap.size());
	      return cost;
	}

	@Override
	public double costPerConfigPerMinute(ExtendedGiraphConfig config) {
		...
		switch(config.getVm_flavor()) {
			...
			default:
				throw new RuntimeException("[ExtendedGiraphConfigCostGenerator] Inexistent vm flavor " + config.getVm_flavor());
		}
	}

	@Override
	public double[] getIntermediateValues(ExtendedGiraphConfig config, int amount, timeout timeoutType) {
		// TODO Auto-generated method stub
		return null;
	}

	@Override
	public double getAccForSpecificTime(double time, ExtendedGiraphConfig config) {
		// TODO Auto-generated method stub
		return 0;
	}
	
	@Override
	public double getAccForConfig(ExtendedGiraphConfig config) {
		// TODO Auto-generated method stub
		return -1;
	}
	...
	private void updateCostOnWorkload(String dataset) throws IOException {
		...
		try{
			...
			System.out.print("[ExtendedGiraphConfigCostGenerator] ");
			 e.printStackTrace();
		}
			...
			if(counter >= 1){			    
			    ...
			    switch(aux_graph){
			    	case "Orkut":
			    		...
			    		break;
			    	default:
			           throw new RuntimeException("[ExtendedGiraphConfigCostGenerator] Unknown graph type " + aux_graph);
			    }
			    ...
			    switch(aux_wkld){
			    	...
			    	case "Connected Components":
			    		...
			    		break;
			    	default:
				        throw new RuntimeException("[ExtendedGiraphConfigCostGenerator] Unknown workload " + aux_wkld);
			    }
			    ExtendedGiraphConfig config = new ExtendedGiraphConfig(GiraphDirectory.vmFlavorFromString(flavor), nr_workers, totalMem, totalCores, graph, wkld);
	...
	public double getTimeForSpecificConstraint(double perc) {
		...
		int i = 0;
		for (Map.Entry<ExtendedGiraphConfig, Double> config : timeMap.entrySet()) {
