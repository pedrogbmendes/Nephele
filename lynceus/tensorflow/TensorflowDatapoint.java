...
public class TensorflowDatapoint extends TensorflowConfig implements Configuration{
	...
	public TensorflowDatapoint(int nr_ps, int nr_workers, double learning_rate, int batch_size, int synchronism, double performance, double accuracy, int vm_flavor, ArrayList<Double> intermediateValues, int nr_iterations){
		super(nr_ps, nr_workers, learning_rate, batch_size, synchronism, vm_flavor);
		this.performance = performance;
		this.accuracy = accuracy;
		if(this.performance > timeOut){
	    	int total_nr_iteration = (int) Math.ceil((Math.log(1.0-0.9))/(1.0*batch_size*Math.log((60000.0-1.0)/60000.0)));
	    	this.performance =  (total_nr_iteration * performance) / nr_iterations;
		}
		
		this.price = this.performance*((vm_price_per_sec(vm_flavor) * (nr_ps + nr_workers))+ vm_price_per_sec(3));
		this.intermediateValues = intermediateValues;
	}
	
	public TensorflowDatapoint(TensorflowConfig c, double performance, double accuracy, int nr_iterations){
		...
		this.price = this.performance *((vm_price_per_sec(c.getVm_flavor()) * (c.getNr_workers() + c.getNr_ps())) + vm_price_per_sec(3));
	}
	...
	public Configuration clone() {
		int nr_iteration = (int) Math.ceil((Math.log(1.0-0.9))/(1.0*batch_size*Math.log((60000.0-1.0)/60000.0)));
		return new TensorflowDatapoint(nr_ps, nr_workers, learning_rate, batch_size, synchronism, performance, accuracy, vm_flavor, intermediateValues, nr_iteration);
	}
