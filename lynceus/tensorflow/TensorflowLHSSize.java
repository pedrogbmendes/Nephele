...
public class TensorflowLHSSize <C extends Configuration> extends LHS{
	
	/* class attributes */
	private ArrayList<double[]> dims;
	
	/* class constructor */
	public TensorflowLHSSize(int samples) {
		super(samples);
	}
	...
	protected ArrayList<double[]> initDims() {
		...
		double[] nr_ps = {1};
		double[] nr_cores = {8, 16, 32, 48, 64, 80};
...
//		double[] nr_workers2XL = {1, 2, 4, 6, 8, 10, 12, 14};
		...
		double[] size = {1.0/60.0, 6.0/60.0, 15.0/60.0, 3.0/6.0, 6.0/6.0};
...
//		this.dims.add(nr_workers2XL);
	...
	protected Configuration newSample(int index) {
		...
		while (i < this.dims.size()) {
			...
			} else if (i == 5){	// selecting nr_ps
				nr_ps = (int) dim[auxIndex];
			} else {
				size = (double) dim[auxIndex];
			}
		...
		TensorflowConfigSize newConfig = new TensorflowConfigSize(nr_ps, nr_workers, learning_rate, batch_size, synchronism, vm_flavor, size);
		//System.out.println("newConfig = " + newConfig);
		
		return new WekaTensorflowConfigSize(newConfig, super.dataset);
	}
	...
	protected ArrayList<Double> dimensions(Configuration config) {
		...
		//size
		if ((double)config.at(6) == 1.0/60.0) {	//size =1000/60000
			dimensionsArray.add(1.0);
			
		} else if ((double)config.at(6) == 6.0/60.0) {	//size =6000/60000
			dimensionsArray.add(2.0);
			
		} else if ((double)config.at(6) == 15.0/60.0) {	//size =15000/60000
			dimensionsArray.add(3.0);
			
		} else if ((double)config.at(6) == 30.0/60.0) {	//size =30000/60000
			dimensionsArray.add(4.0);
			
		} else {								//size =60000/60000
			dimensionsArray.add(5.0);
